function [Ahat,es,ts,status] = Procrustes_CARD(X,B,options)
%% Procrustes_CARD:
% Same as Procrustes_ANFGM, but computes Y_diamond in the post-processing. 
% It implements CARD method.
% See Procrustes_ANFGM for further info.
...
[U,D,V]=svd(X);
d=diag(D);
if options.rank==0  % describes if the original matrix X is truncated
...
if options.nspsd == 1 % NSPSD PROCRUSTES SOLVED; see [BGP21]
    ...
    Y=Z*W1;
    YD=Y*sqrtm(inv(Lambda));
    p=8*norm(Y,'fro')^2/norm(YD'*YD,'fro')^2;
    alpha=Cardano(p,-p);
    Y=alpha*Y;        
    N=0.25*alpha^2*(YD*YD');
    M=W1*Y'-Z';
