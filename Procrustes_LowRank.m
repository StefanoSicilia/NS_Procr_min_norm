function [Ahat,es,ts,status] = Procrustes_LowRank(X,B,options)
%% Procrustes_LowRank:
% Follows procrustes_anly but the output matrix Ahat is built differently.
% In this way it is possible to avoid the blow up of the solution's norm.
...
if options.nspsd == 1 % NSPSD PROCRUSTES SOLVED; see [BGP21]
    ...
    if ~any(temp(:)>0)
        ...
        if ~any(eig(H11)<10^-9) || norm(Z*null(H11),'fro')<1e-6
            ...
            Heps = U11*Heps*U11';
            Aeps = Heps + S11;
            Ahat=U1*Aeps*U1'+4*skew(U2*Z*U1');
            ubound = norm(Ahat*X-B,'fro');
